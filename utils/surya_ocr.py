...
def run(file_path: Path, language: str = 'sa', additional_languages: Optional[List[str]] = None) -> OcrResponse:
    ...
    try:
        ...
        logging.info(f"Running Surya OCR with automatic language detection")
        ...
        err = str(e)
        # Prebuilt wheels (e.g. flash-attn) may need a newer glibc than the host (Ubuntu 20.04 has 2.31).
        _native_loader_issue = any(
            token in err
            for token in (
                "GLIBC_",
                "flash_attn",
        ...
        if _native_loader_issue:
            logging.error(
                "Surya OCR failed to load a native library (%s). This is often flash-attn "
                "built for newer glibc than this OS. Options: Ubuntu 22.04+ or Docker, "
                "build flash-attn from source on this machine, or pip uninstall flash_attn "
                "if your stack allows Surya without it.",
                e,
            )
            ...
            try:
                ...
                raise RuntimeError(
                    f"Surya OCR native load failed: {e}. Tesseract fallback failed: {fallback_error}"
                ) from fallback_error
