...
Celery processes tasks across dedicated queues (`default`, `pdf_processing`, `ocr`, `translation`, `low_priority`, `s3_batch`, `search_index`, `metadata`) partitioned across autoscaling worker containers (`kalanjiyam-celery`, `kalanjiyam-celery-pdf`, `kalanjiyam-celery-ocr`, `kalanjiyam-celery-translation`, `kalanjiyam-celery-batch`, `kalanjiyam-celery-metadata`). Priority levels (0-9) prevent large batch jobs from starving interactive editor requests. OpenSearch (`kalanjiyam-search`) handles full-text search indexing. OCR runs as a separate microservice; set `OCR_SERVICE_URL` to a host reachable from inside containers.
