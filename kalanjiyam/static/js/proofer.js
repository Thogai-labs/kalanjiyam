...
export default () => ({
  ...
  getVersionDisplayName(versionKey) {
    ...
    if (versionKey.startsWith('ocr:enhanced:')) {
      ...
      const engineMap = {
        ...
        "bodhan": "2",
        "chandra": "3",
        "tesseract_manuscript": "4",
        "sanskrit_manuscript": "4",
        "sanskrit-manuscript": "4",
        "dots_ocr": "5",
        "dots-ocr": "5",
        "5": "5",
        "12": "5",
  ...
  decodeEngine(engineValue) {
    const engineMap = {
      ...
      '5': 'dots_ocr',
      // Legacy backwards-compatible numeric mappings
      '6': 'chandra',
    ...
    return engineMap[engineValue] || engineValue || 'gemma_ocr';
  },
  ...
  decodeTranslationEngine(engineValue) {
    ...
    return engineMap[engineValue] || engineValue || 'indictrans3';
  },
  ...
  getOcrEngineName(engineValue) {
    ...
    const engineMap = {
      ...
      "5": "5",
      "12": "5",
      // Legacy numeric lookups mapped to new numbers
      "13": "1",
      "14": "2",
      "6": "3",
      "11": "4"
    ...
    if (val && /^\d+$/.test(val)) {
      const mapped = engineMap[val] || val;
      return 'OCR ' + mapped;
    }
    ...
    const decodedNum = engineMap[decoded] || (/^\d+$/.test(decoded) ? (engineMap[decoded] || decoded) : null);
    if (decodedNum) {
