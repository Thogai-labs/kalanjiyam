...
class DeepSeekOcrEngine:
    ...
    def _load_model(self):
        """Load the DeepSeek OCR model and tokenizer with GPU-first, CPU fallback."""
        try:
            ...
            if device == 'cuda':
                ...
                logging.info("Model loaded on CUDA GPU with bfloat16 precision")
            else:
                self.model = self.model.eval().to(torch.float32)
                logging.info("Model loaded on CPU with float32 precision")
            
            logging.info(f"DeepSeek OCR model loaded successfully on {device}")
